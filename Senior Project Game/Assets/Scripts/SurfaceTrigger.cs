...
public class SurfaceTrigger : MonoBehaviour {
    ...
    public void Create(Surface parentSurface, SurfaceTriggerType type, bool doubleSide = false) {
        ...
        if(triggerType == SurfaceTriggerType.Motion) {
            ...
            collider.transform.localPosition = new Vector3(0, Mathf.Max(parentSurface.boxCollider.size.y, 0.5f) * 0.5f * (doubleSide ? -1 : 1), 0);
            ...
            collider.transform.localPosition = new Vector3(0, parentSurface.boxCollider.size.y * -0.25f * (doubleSide ? -1 : 1), 0);
        }

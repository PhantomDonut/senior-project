...
public class ObjectPoolManager : MonoBehaviour {
    ...
    [SerializeField] private Vector3 defaultPosition = default;

    public void Start() {
        coinPool.pooledObjects = new Queue<IPoolable>(coinPool.amount * 2);
        GameObject tempReference;
        Transform parentObject = new GameObject("Coin Pool").transform;
        parentObject.transform.position = defaultPosition;
        parentObject.SetParent(transform);
        for (int i = 0; i < coinPool.amount; i++) {
            tempReference = Instantiate(coinPool.prefab, defaultPosition, Quaternion.identity, parentObject);
